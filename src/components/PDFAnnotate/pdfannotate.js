	...
	try {
		if (fileid == ''){
			...
			$.each(inst.fabricObjects, function (index, fabricObj) {
				console.log(index);
				var leadsRef = firebase.database().ref(useridother + '/Documents/'+fileid + '/Data/'+ index)
	...
	task.on('state_changed', function progress(snapshot) {
	  ...
	  console.log(url)
		...
		$.each(inst.fabricObjects, function (index, fabricObj) {
			...
			firebase.database().ref(useridother+'/Documents/'+filename+'/Data/').child(index).set(JSON.stringify(jsonData[index]));
		...
		var ref = firebase.database().ref(useridother + '/Documents/'+filename+'/Reciever/');
              ref.orderByChild('RecepientEmail').equalTo(email).on("value", function(snapshotchild) {
				totalcount = snapshotchild.numChildren();
				...
				indexref.orderByChild('RecepientStatus').equalTo('Completed').on("value", function(snapshotchild) {
				  ...
				  indexref.on("value", function(snapshotchild) {
					console.log(snapshotchild.numChildren());
					totalcount = snapshotchild.numChildren();
				});
				...
				Email.send({
				  ...
				  message => {
					...
					firebase.database().ref(useridother + '/Documents/'+filename+'/').child('Status').set('Completed');
					firebase.database().ref(useridother + '/Documents/'+filename+'/').child('DateStatus').set(today);
					}
...
$(document).on('click','.actionsign', function() {  
  ...
  $(document).mousemove(function(e){
		$("#dragabbleImage").css({left:e.pageX - 200, top:e.pageY-200});
  });
  $("#dragabbleImage").hide();
}
...
render() {
	...
	<img id="dragabbleImage" src={require('../../assets/img/icons/common/drop.svg')} />
