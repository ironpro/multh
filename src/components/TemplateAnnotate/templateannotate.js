	...
	task.on('state_changed', function progress(snapshot) {
	  ...
	  console.log(url)
		...
		$.each(inst.fabricObjects, function (index, fabricObj) {
			...
			firebase.database().ref(userid + '/Templates/'+filename+'/Data/').child(index).set(JSON.stringify(jsonData[index]));
			modal[1].style.display = "none";
			
		})
		url = 'https://pappayasign.surge.sh/#/admin/sign?id=' + encodeURIComponent(filename) + '&type=db&u=' + userid;
		
		firebase.database().ref(userid + '/Templates/'+filename).child('OwnerEmail').set(email);
		firebase.database().ref(userid + '/Templates/'+filename).child('Status').set('Draft');
		firebase.database().ref(userid + '/Templates/'+filename).child('TemplateName').set(inst.filename);
		firebase.database().ref(userid +  '/Templates/'+filename).child('Owner').set(userid);
		firebase.database().ref(userid +  '/Templates/'+filename).child('DateCreated').set(today);
		firebase.database().ref(userid +  '/Templates/'+filename).child('DateStatus').set(today);
		...
		people.forEach(function(item, index) {
			...
			console.log(recepientEmail + ',' + recepientName);
				  ...
				  firebase.database().ref(userid + '/Templates/'+filename+'/').child('Status').set('Waiting for Others');
				  firebase.database().ref(userid + '/Templates/'+filename+'/Reciever/'+index).child('RecepientName').set(recepientName);
				  firebase.database().ref(userid + '/Templates/'+filename+'/Reciever/'+index).child('DocumentName').set(inst.filename);
				  firebase.database().ref(userid + '/Templates/'+filename+'/Reciever/'+index).child('RecepientEmail').set(recepientEmail);
				  firebase.database().ref(userid + '/Templates/'+filename+'/Reciever/'+index).child('RecepientColor').set(recepientColor);
				  firebase.database().ref(userid + '/Templates/'+filename+'/Reciever/'+index).child('RecepientOption').set(recepientOption);
				  firebase.database().ref(userid + '/Templates/'+filename+'/Reciever/'+index).child('RecepientStatus').set('Sent');
				  firebase.database().ref(userid + '/Templates/'+filename+'/Reciever/'+index).child('RecepientDateStatus').set(today);
